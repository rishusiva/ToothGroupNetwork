...
import numpy as np
from models import bdl_grouping_netowrk_model
from models import fps_grouping_network_model
...
class InferencePipeLine:
    def __init__(self, config):
        ...
        self.first_module.load_state_dict(torch.load(self.config["fps_model_info"]["load_ckpt_path"]+".h5", map_location=torch.device('cpu')))

        self.bdl_module = GroupingNetworkModule(self.config["boundary_model_info"])
        self.bdl_module.load_state_dict(torch.load(self.config["boundary_model_info"]["load_ckpt_path"]+".h5", map_location=torch.device('cpu')))

    def __call__(self, stl_path):
        DEBUG = False
        _, mesh = gu.read_txt_obj_ls(stl_path, ret_mesh=True, use_tri_mesh=True)
        ...
        input_cuda_feats = torch.from_numpy(np.array([sampled_feats.astype('float32')])).permute(0, 2, 1)
        ...
        input_cuda_bdl_feats = torch.from_numpy(np.array([sampled_boundary_feats.astype('float32')])).permute(0, 2, 1)
        sampled_boundary_seg_label = torch.from_numpy(np.array([sampled_boundary_seg_label.astype(int)])).permute(0, 2, 1) - 1
        bdl_results = self.get_second_module_results(input_cuda_bdl_feats, sampled_boundary_seg_label, self.bdl_module)

        if DEBUG:
            gu.print_3d(gu.np_to_pcd_with_label(first_results["ins"]["full_ins_labeled_points"]), gu.np_to_pcd_with_label(bdl_results["ins"]["full_ins_labeled_points"]))
        ...
        first_ps_label_unique_except_zero = np.unique(first_ps_label)
        first_ps_label_unique_except_zero = first_ps_label_unique_except_zero[first_ps_label_unique_except_zero != 0]
        ...
        pca.fit(ins_label_center_points)
        ...
        for ins_label in np.unique(first_ps_label):
            ...
            if max_freq_first_cluster_label not in [1, 9]:
                if np.dot(ins_points_center - centerpoints_of_11_12, checking_axis_vector) < 0:
                    max_freq_first_cluster_label = max_freq_first_cluster_label + 8
    ...
    def get_first_module_results(self, feats, base_model):
        ...
        whole_pd_mask_2 = torch.zeros((points.shape[2], 2))
        whole_pd_mask_count_2 = torch.zeros(points.shape[2])
